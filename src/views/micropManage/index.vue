<template>
    <a-layout-content>
         <a-button type="primary" class="mainButton" @click="showModal(-1)">添加小百科</a-button>
     <table>
         <tr>
            <td>序号</td>
            <td>名称</td>
          ...
          <tr v-for="(item, index) in micropList" :key="index">
...
<script>
import MicropModel from './MicropModel'
export default {
    components:{
        MicropModel
    },
    data () {
      ...
      methods: {
          getMicropList(){
            this.$axios({
              url:"/admin/food/queryFoodClassifyList",
            }).then(res=>{
                console.log(res)
               this.micropList=res.data.data
            })
          },
         showModal(id) {
            this.idx=id;
